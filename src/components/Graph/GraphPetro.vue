<template>
  <div>
    <app-card heading="Petro" :headingMenu="true" v-if="loadedGraphPetro">
      <!-- Precios del dia -->
      ...
      <line-chart
        ...
        ref="monedaPetro"
        :chartData="dataPetro"
        :options="optionsPetro"
        :styles="styles"
      ></line-chart>
    </app-card>
    <div v-else class="d-flex justify-content-center py-5 mb-5">
      <fade-loader :loading="!loadedGraphPetro" color="DarkOrange" size="32px"></fade-loader>
    </div>
  </div>
</template>
<script>
      ...
      return {
        ...
        scales: {
          xAxes: [
            {
              ticks: {
                min: this.$store.state.tasas.minPetroX,
                max: this.$store.state.tasas.maxPetroX,
              }
            }
          ],
          yAxes: [
            {
              ticks: {
                min: this.$store.state.tasas.minPetroY,
                max: this.$store.state.tasas.maxPetroY,
                // stepSize: 20000
